...
import { Plus, Search, MoreHorizontal, Edit, Trash2, Phone, Mail, MapPin, Loader2 } from "lucide-react";
import {
...
export default function ProvidersPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await supabase.from("providers").delete().eq("id", deleteTarget);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button className="bg-sage hover:bg-sage-dark" onClick={() => router.push("/providers/new")}>
                  ...
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => router.push(`/providers/${p.id}`)}>
                          <Edit className="mr-2 h-4 w-4" /> Edit
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => handleContact(p.email)}>
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(open) => { if (!open) setDeleteTarget(null); }}>
        <DialogContent className="max-w-sm">
          ...
          <p className="text-sm text-muted-foreground">
            Are you sure you want to remove &quot;{providers.find((p) => p.id === deleteTarget)?.full_name}&quot;?
          </p>
          <DialogFooter>
